...
class PyCSV:
    ...
    def split_csv(self, csv_path, save_dir, csv_encoding='utf-8'):
        ...
        self.line_numbers = 0
        # 获取数据行数
        self.total = sum(1 for line in open(File_Train))
        # 确定每个文件所有的行数
    ...
    def split_csv(self, path):
        ...
        with open(path, 'r', newline='') as file:
            ...
            for df in df_iter:
                ...
                # 设置切分后文件的保存路径
                if (i == 1):
                    save_filename = os.path.join(File_Train)
                else:
                    save_filename = os.path.join(File_Check)
                # 打印保存信息
                print(f"{save_filename} 已经生成！")
                # 保存切分后的数据
                df.to_csv(save_filename, index=False, encoding='utf-8', quoting=1)
...
def get_label_data():
    # 删除上次的结果
    if os.path.exists(File_Upset):
        os.remove(File_Upset)
    data = pd.read_csv(File_Name, sep=',')
    data = shuffle(data)
    data.to_csv(File_Upset, index=False, header=True)
